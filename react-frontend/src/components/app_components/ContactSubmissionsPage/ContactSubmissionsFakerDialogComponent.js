...
import contactSubmissionsFakerFactory from "./contactSubmissionsFakerFactory";
...
const ContactSubmissionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contactSubmissionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contactSubmissions").create(elem));
...
export default connect(mapState, mapDispatch)(ContactSubmissionsFakerDialogComponent);
