...
import ContactSubmissionsDatatable from "./ContactSubmissionsDataTable";
...
import ContactSubmissionsFakerDialogComponent from "./ContactSubmissionsFakerDialogComponent";
import ContactSubmissionsSeederDialogComponent from "./ContactSubmissionsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ContactSubmissionsPage = (props) => {
    ...
    const filename = "contactSubmissions";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("contactSubmissions");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Contact Submissions", type: "error", message: error.message || "Failed get Contact Submissions" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("contactSubmissions").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Contact Submissions", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("contactSubmissions").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/contactSubmissions/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="contactSubmissions-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="contactSubmissions-datatable">
                    <ContactSubmissionsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ContactSubmissionsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ContactSubmissionsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ContactSubmissionsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ContactSubmissionsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ContactSubmissionsPage);
