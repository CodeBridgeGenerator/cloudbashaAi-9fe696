...
const ContactSubmissionsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,contactNo: _entity?.contactNo,email: _entity?.email,companyName: _entity?.companyName,position: _entity?.position,message: _entity?.message,user: _entity?.user,paymentStatus: _entity?.paymentStatus,paymentIntentId: _entity?.paymentIntentId,sessionId: _entity?.sessionId,amount: _entity?.amount,currency: _entity?.currency,paidAt: _entity?.paidAt,status: _entity?.status,
        ...
        const result = await client.service("contactSubmissions").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Contact Submissions created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Contact Submissions" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Contact Submissions" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="contactSubmissions-create-dialog-component">
...
export default connect(mapState, mapDispatch)(ContactSubmissionsCreateDialogComponent);
