...
import blogsFakerFactory from "./blogsFakerFactory";
...
const BlogsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = blogsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("blogs").create(elem));
...
export default connect(mapState, mapDispatch)(BlogsFakerDialogComponent);
