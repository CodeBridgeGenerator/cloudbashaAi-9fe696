...
import headersFakerFactory from "./headersFakerFactory";
...
const HeadersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = headersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("headers").create(elem));
...
export default connect(mapState, mapDispatch)(HeadersFakerDialogComponent);
