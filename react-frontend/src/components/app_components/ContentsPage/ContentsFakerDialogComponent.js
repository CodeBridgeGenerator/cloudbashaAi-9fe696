...
import contentsFakerFactory from "./contentsFakerFactory";
...
const ContentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contentsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contents").create(elem));
...
export default connect(mapState, mapDispatch)(ContentsFakerDialogComponent);
