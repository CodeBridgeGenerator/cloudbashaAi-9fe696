...
import emailSubscriptionsFakerFactory from "./emailSubscriptionsFakerFactory";
...
const EmailSubscriptionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = emailSubscriptionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("emailSubscriptions").create(elem));
...
export default connect(mapState, mapDispatch)(EmailSubscriptionsFakerDialogComponent);
