...
import formGeneratorFakerFactory from "./formGeneratorFakerFactory";
...
const FormGeneratorFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = formGeneratorFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("formGenerator").create(elem));
...
export default connect(mapState, mapDispatch)(FormGeneratorFakerDialogComponent);
