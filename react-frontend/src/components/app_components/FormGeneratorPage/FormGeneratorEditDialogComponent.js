...
const FormGeneratorEditDialogComponent = (props) => {
...
generatedQuestions: _entity?.generatedQuestions,
formResponses: _entity?.formResponses,
conversationHistory: _entity?.conversationHistory,
dashboardData: _entity?.dashboardData,
rawGeminiResponse: _entity?.rawGeminiResponse,
generationError: _entity?.generationError,
user: _entity?.user,
collaborators: _entity?.collaborators,
isPublic: _entity?.isPublic,
publicUrl: _entity?.publicUrl,
        };

        setLoading(true);
        try {
            
        const result = await client.service("formGenerator").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info formGenerator updated successfully" });
    ...
    return (
        <Dialog header="Edit Form Generator" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="formGenerator-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(FormGeneratorEditDialogComponent);
