...
import phrasesFakerFactory from "./phrasesFakerFactory";
...
const PhrasesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = phrasesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("phrases").create(elem));
...
export default connect(mapState, mapDispatch)(PhrasesFakerDialogComponent);
