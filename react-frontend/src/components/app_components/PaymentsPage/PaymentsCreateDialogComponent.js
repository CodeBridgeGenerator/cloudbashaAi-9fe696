...
const PaymentsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            paymentStatus: _entity?.paymentStatus,paymentIntentId: _entity?.paymentIntentId,sessionId: _entity?.sessionId,amount: _entity?.amount,currency: _entity?.currency,paidAt: _entity?.paidAt,status: _entity?.status,
