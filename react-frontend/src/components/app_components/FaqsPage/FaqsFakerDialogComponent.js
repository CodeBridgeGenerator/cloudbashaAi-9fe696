...
import faqsFakerFactory from "./faqsFakerFactory";
...
const FaqsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = faqsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("faqs").create(elem));
...
export default connect(mapState, mapDispatch)(FaqsFakerDialogComponent);
