...
import bonusFakerFactory from "./bonusFakerFactory";
...
const BonusFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = bonusFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bonus").create(elem));
...
export default connect(mapState, mapDispatch)(BonusFakerDialogComponent);
