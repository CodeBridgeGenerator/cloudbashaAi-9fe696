...
import apiTokensFakerFactory from "./apiTokensFakerFactory";
...
const ApiTokensFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = apiTokensFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("apiTokens").create(elem));
...
export default connect(mapState, mapDispatch)(ApiTokensFakerDialogComponent);
