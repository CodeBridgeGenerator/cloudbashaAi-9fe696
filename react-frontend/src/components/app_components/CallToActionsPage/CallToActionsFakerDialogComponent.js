...
import callToActionsFakerFactory from "./callToActionsFakerFactory";
...
const CallToActionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = callToActionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("callToActions").create(elem));
...
export default connect(mapState, mapDispatch)(CallToActionsFakerDialogComponent);
