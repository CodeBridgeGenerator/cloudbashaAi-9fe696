...
import featureRequestsFakerFactory from "./featureRequestsFakerFactory";
...
const FeatureRequestsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = featureRequestsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("featureRequests").create(elem));
...
export default connect(mapState, mapDispatch)(FeatureRequestsFakerDialogComponent);
