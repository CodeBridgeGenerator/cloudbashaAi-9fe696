...
import careersFakerFactory from "./careersFakerFactory";
...
const CareersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = careersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("careers").create(elem));
...
export default connect(mapState, mapDispatch)(CareersFakerDialogComponent);
