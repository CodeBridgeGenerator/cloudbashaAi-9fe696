const { FeatureRequests } = require("./featureRequests.class");
const createModel = require("./featureRequests.model");
const hooks = require("./featureRequests.hooks");
...
module.exports = function (app) {
  ...
  app.use("/featureRequests", new FeatureRequests(options, app));

  // Get our initialized service so that we can register hooks
  const service = app.service("featureRequests");

  // Get the schema of the collections 
  app.get("/featureRequestsSchema", function (request, response) {
